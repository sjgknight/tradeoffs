...
export function updateSelections(store) {
    ...
    if (moves?.length === 1 && moves[0].selections.length === 1) {
        if (!move)
            move = decorateUIMove(moves[0]);
        const selection = moves[0].selections[0];
        if (selection.type === 'place' && !placement) {
            let piece = selection.clientContext.placement.piece;
            if (typeof piece === 'string')
                piece = moves[0].args[piece];
            const into = selection.clientContext.placement.into;
    ...
    if (maySubmit && moves?.length === 0) {
        if (move) {
            ...
            if (player) {
                ...
                console.debug(`${autoSubmit ? 'Autoplay' : 'Submitting'} valid move from player #${position}:\n` +
                ...
                if (autoSubmit) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!(e instanceof NoRandomAllowed))
                                throw e;
                        }
                        finally {
    ...
    if (gameManager.players.currentPosition.length > 0) {
        ...
        if (actionsWithDescription.length === 1) {
            ...
            if (!gameManager.players.currentPosition.includes(position))
                state.otherPlayerAction = actionsWithDescription[0].name;
        }
...
export function updateControls(store) {
    ...
    if (!layout && move && !pendingMoves?.[0].selections[0].isBoardChoice()) {
        const element = Object.entries(move.args).reverse().find(([name, el]) => (!gameManager.game._ui.stepLayouts["action:" + move.name]?.noAnchor?.includes(name) && el instanceof GameElement));
        if (element && store.rendered.all[element[1]._t.ref]) {
            layout = { element: element[1], position: 'beside', gap: 2 };
    ...
    const box = store.rendered.all[layout?.element._t.ref]?.pos;
    if (layout && box) {
        if (layout.position === 'beside' || layout.position === 'stack') {
            ...
            if (layout.right !== undefined) {
                ...
                style.left ?? (style.left = ((layout.center - 50) * box.width / 100) + box.left + '%');
                style.right = 100 + ((50 - layout.center) * box.width / 100) - box.left - box.width + '%';
                style.margin = '0 auto';
            }
            else {
                style.left ?? (style.left = ((layout.left ?? 0) * box.width / 100) + box.left + '%');
            }
        ...
        if (layout.width !== undefined)
            style.maxWidth = (layout.width * box.width / 100) + '%';
        if (layout.height !== undefined)
            style.maxHeight = (layout.height * box.height / 100) + '%';
    }
    else {
...
export function updateBoardSelections(store) {
    var _a, _b, _c, _d;
        ...
        for (const sel of p.selections) {
            if (sel.type === 'board' && sel.boardChoices) {
                ...
                for (const el of boardChoices) {
                    boardSelections[_a = el.branch()] ?? (boardSelections[_a] = { clickMoves: [], dragMoves: [] });
                    boardSelections[el.branch()].clickMoves.push(boardMove);
                }
                for (const { option, error } of sel.invalidOptions) {
                    boardSelections[_b = option.branch()] ?? (boardSelections[_b] = { clickMoves: [], dragMoves: [] });
                    boardSelections[option.branch()].error = error;
                }
                let { dragInto, dragFrom } = sel.clientContext;
                if (dragInto) {
                    ...
                    for (const el of boardChoices) {
                        boardSelections[_c = el.branch()] ?? (boardSelections[_c] = { clickMoves: [], dragMoves: [] });
                ...
                if (dragFrom) {
                    for (const el of dragFrom instanceof GameElement ? [dragFrom] : dragFrom.resolve(move?.args || {}).boardChoices || []) {
                        boardSelections[_d = el.branch()] ?? (boardSelections[_d] = { clickMoves: [], dragMoves: [] });
...
export function updatePrompts(store) {
    const { controls, prompt, actionDescription, gameManager, position, disambiguateElement } = store;
    if (actionDescription && !gameManager.players.currentPosition.includes(position))
        return { boardPrompt: actionDescription };
    if (!controls)
