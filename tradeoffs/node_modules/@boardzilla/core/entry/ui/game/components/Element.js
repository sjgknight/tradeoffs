...
const defaultAppearance = (el) => React.createElement("div", { className: "bz-default" }, el.toString());
const Element = ({ render, mode, onSelectElement, onMouseLeave }) => {
    const [rendered, setInfoElement, setError, dragElement, setDragElement, dragOffset, dropSelections, currentDrop, setCurrentDrop, placement, setPlacement, selectPlacement, isMobile, dev] = gameStore(s => [s.rendered, s.setInfoElement, s.setError, s.dragElement, s.setDragElement, s.dragOffset, s.dropSelections, s.currentDrop, s.setCurrentDrop, s.placement, s.setPlacement, s.selectPlacement, s.isMobile, s.dev]);
    ...
    const [dragging, setDragging] = useState(); // currently dragging
    ...
    const gridSizeNeeded = useMemo(() => (placement?.into._sizeNeededFor(placement.piece) ?? { width: 1, height: 1 }
    ...
    useEffect(() => {
        ...
        if (node?.style.getPropertyValue('--transformed-to-old')) {
            ...
            if (domElement.current && render.previousDataAttributes) {
                for (const [k, v] of Object.entries(render.dataAttributes))
                    domElement.current.setAttribute(k, v);
            ...
            if (node.style.getPropertyValue('transform')) {
                ...
                if (render.crossParent)
                    node.classList.add('cross-parent');
                ...
                if (render.styles?.transform)
                    delete render.styles.transform;
            }
        }
        delete render.mutated;
    }, [element, render]);
    useEffect(() => {
        if (dragging && dragElement !== branch)
        ...
        if (dragging) {
            if (currentDrop) {
                ...
                if (move) {
                    ...
                    wrapper.current.style.top = render.styles.top;
                    wrapper.current.style.left = render.styles.left;
                }
            }
            setDragging(undefined);
    ...
    const handlePlacement = useCallback((event) => {
        ...
        if (placement.piece.row === undefined || placement.piece.column === undefined) {
            ...
            if (pointer.column < placement.piece.column) {
                ...
                newPlacement.column = Math.min(grid.origin.column + grid.columns - gridSizeNeeded.width, Math.floor(pointer.column - gridSizeNeeded.width + 1));
            }
            if (pointer.row < placement.piece.row) {
                ...
                newPlacement.row = Math.min(grid.origin.row + grid.rows - gridSizeNeeded.height, Math.floor(pointer.row - gridSizeNeeded.height + 1));
            }
        }
        if (newPlacement.column !== undefined) {
            newPlacement.column = Math.max(grid.origin.column, Math.min(grid.origin.column + grid.columns - gridSizeNeeded.width, Math.floor(newPlacement.column)));
        }
        if (newPlacement.row !== undefined) {
            newPlacement.row = Math.max(grid.origin.row, Math.min(grid.origin.row + grid.rows - gridSizeNeeded.height, Math.floor(newPlacement.row)));
        }
    ...
    const handleRotate = useCallback((direction) => {
        ...
        if (!choices)
            return;
        let rotation = choices[(choices.indexOf(element.rotation) + direction + choices.length) % placement.rotationChoices.length];
    ...
    let contents = [];
    const containerPendingContents = {};
    for (let l = 0; l !== render.layouts.length; l++) {
        ...
        for (const render of layout.children) {
            if (render.proxy !== undefined && !render.mutated)
                continue;
            layoutContents.push(React.createElement(Element, { key: render.key, render: render, mode: mode === 'zoom' ? 'game' : mode, onMouseLeave: droppable ? () => setCurrentDrop(element) : undefined, onSelectElement: onSelectElement }));
        }
        if (layout.container) {
            if (layout.container.id) {
                containerPendingContents[layout?.container.key ?? 'main'] = layoutContents;
                if (render.layouts.find((layout2, l2) => l2 > l && layout2.container?.id === layout.container?.id))
                    continue;
            }
            const component = { drawer: Drawer, popout: Popout, tabs: Tabs }[layout.container.type];
            ...
            if (layoutContents.length)
                contents.push(React.createElement("div", { key: l, className: "layout-wrapper" }, layoutContents));
        }
    }
    if (element._ui.appearance.connections && '_graph' in element) {
        ...
        const lines = [];
        const labels = [];
        element._graph.forEachEdge((...args) => {
            ...
            if (source && target) {
                ...
                if (style === 'double') {
                    lines.push(React.createElement("line", { key: i++, className: "outer", x1: origin.x, y1: origin.y, x2: destination.x, y2: destination.y, transform: `translate(${(origin.y - destination.y) / distance * thickness}, ${(origin.x - destination.x) / distance * -thickness})`, strokeWidth: thickness, stroke: color }));
                    lines.push(React.createElement("line", { key: i++, className: "outer", x1: origin.x, y1: origin.y, x2: destination.x, y2: destination.y, transform: `translate(${(origin.y - destination.y) / distance * -thickness}, ${(origin.x - destination.x) / distance * thickness})`, strokeWidth: thickness, stroke: color }));
                }
                lines.push(React.createElement("line", { key: i++, className: "inner", x1: origin.x, y1: origin.y, x2: destination.x, y2: destination.y, strokeWidth: 2 * thickness, stroke: fill }));
                if (label) {
                    labels.push(React.createElement("g", { key: `label${i}`, transform: `translate(${(origin.x + destination.x) / 2 - labelScale * absolutePosition.width * .5}
  ${(origin.y + destination.y) / 2 - labelScale * absolutePosition.height * .5})
  scale(${labelScale})` }, label({ distance: args[1].distance, to: args[4].space, from: args[5].space })));
                }
            }
        });
        contents.unshift(React.createElement("svg", { key: "svg-edges", style: { pointerEvents: 'none', position: 'absolute', width: '100%', height: '100%', left: 0, top: 0 }, viewBox: `0 0 ${absolutePosition.width} ${absolutePosition.height}` }, lines));
        if (label)
            contents.push(React.createElement("svg", { key: "svg-edge-labels", style: { pointerEvents: 'none', position: 'absolute', width: '100%', height: '100%', left: 0, top: 0 }, viewBox: `0 0 ${absolutePosition.width} ${absolutePosition.height}` }, labels));
    }
    const boundingBoxes = render.layouts?.filter(layout => layout.showBoundingBox).map((layout, k) => (React.createElement("div", { key: k, className: "bz-show-grid", style: {
        ...
        } }, typeof layout.showBoundingBox === 'string' && React.createElement("span", null, layout.showBoundingBox))));
    let title = undefined;
    if (dev) {
...
${Object.entries(element.attributeList()).filter(([k, v]) => v !== undefined && !['_size', '_visible', 'was'].includes(k)).map(([k, v]) => `  ${k}: ${typeof v === 'object' && ('isGameElement' in v.constructor || 'isPlayer' in v.constructor) ? v.toString() : JSON.stringify(v)}`).join("\n")}`;
    ...
    contents = (React.createElement("div", { id: element.name, ref: domElement, title: title, className: classNames(render.classes, {
            ...
            selectable,
            droppable
        }), style: render.baseStyles, onClick: clickable || placing || invalidSelectionError ? handleClick : undefined, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, onMouseMove: placement?.into === element && !placement.selected ? handlePlacement : undefined, ...attrs },
        appearance(element),
        boundingBoxes,
        contents));
    // wrapper dom element for transforms and animations
    contents = (React.createElement("div", { ref: wrapper, className: classNames("transform-wrapper", { dragging, placing, 'has-info': !!info }), style: styles },
        contents,
        !!info && React.createElement("div", { className: "info-hotspot", onClick: () => setInfoElement({ info, element }) })));
    if (placing && placement?.rotationChoices) {
        ...
        contents = (React.createElement(React.Fragment, null,
            contents,
            React.createElement("div", { className: "rotator", style: { ...styles, width: (render.relPos.width * widthStretch) + '%', fontSize: (0.08 * minSquare) + 'rem', transform: undefined } },
                React.createElement("div", { className: "left", onClick: () => handleRotate(-1) },
                    React.createElement("svg", { viewBox: "0 0 254.2486 281.95978", xmlns: "http://www.w3.org/2000/svg" },
                        React.createElement("g", { transform: "translate(43.69768,-47.016626)" },
                            React.createElement("path", { d: "m 135.24991,227.00186 -76.975589,76.97558 -76.97559,-76.97558 53.43953,0.48619 c 0,-102.89725 56.23959,-155.471424 150.812659,-155.471424 l -0.0693,38.606184 c -67.82216,0 -113.184769,38.35922 -113.184769,117.17431 z" })))),
                React.createElement("div", { className: "right", onClick: () => handleRotate(1) },
                    React.createElement("svg", { viewBox: "0 0 254.2486 281.95978", xmlns: "http://www.w3.org/2000/svg" },
                        React.createElement("g", { transform: "translate(210,-47.016626) scale(-1 1)" },
                            React.createElement("path", { d: "m 135.24991,227.00186 -76.975589,76.97558 -76.97559,-76.97558 53.43953,0.48619 c 0,-102.89725 56.23959,-155.471424 150.812659,-155.471424 l -0.0693,38.606184 c -67.82216,0 -113.184769,38.35922 -113.184769,117.17431 z" })))))));
    }
    if (!isMobile && element instanceof Piece) {
        contents = (React.createElement(DraggableCore, { disabled: !draggable, onStart: handleDragStart, onDrag: handleDrag, onStop: handleDragStop }, contents));
    }
    return contents;
}; // , (prevProps, props) => {
